...
class MySQLFeatureStore:
    """
    MySQL-based feature store for Karachi AQI data
    """
    ...
    def _connect(self):
        ...
        try:
            ...
            if self.connection.is_connected():
                ...
                # Ensure autocommit is on if we want immediate writes without explicit commits
    ...
    def _create_table(self):
        ...
        if not self.is_connected:
            logger.error("❌ Not connected to MySQL, cannot create table.")
            return
    ...
    def insert_data(self, data_df: pd.DataFrame) -> bool:
        ...
        if not self.is_connected:
            logger.error("❌ Not connected to MySQL, cannot insert data.")
            return False
        ...
        try:
            cursor = self.connection.cursor()
            # Prepare columns and placeholders dynamically from DataFrame columns
            # We assume all DataFrame columns correspond to table columns for simplicity
            # For robust production, map DataFrame columns to table schema explicitly
            ...
            update_fields = ', '.join([f'{col}=VALUES({col})' for col in data_df.columns if col != 'timestamp'])
            ...
            logger.info(f"✅ Inserted/Updated {cursor.rowcount} records into MySQL")
            return True

        except Error as e:
            logger.error(f"❌ Error inserting/updating data into MySQL: {e}")
            return False

    def get_recent_data(self, hours: int = 24) -> pd.DataFrame:
        """Get recent data from feature store"""
        if not self.is_connected:
            logger.error("❌ Not connected to MySQL, cannot retrieve data.")
            return pd.DataFrame()
        ...
        try:
            ...
            if records:
                ...
                logger.info(f"✅ Retrieved {len(df)} recent records from MySQL")
                return df
            else:
                logger.warning("⚠️ No recent data found in MySQL")
                return pd.DataFrame()

        except Error as e:
            logger.error(f"❌ Error retrieving recent data from MySQL: {e}")
            return pd.DataFrame()

    def get_historical_data(self, days: int = 30) -> pd.DataFrame:
        """Get historical data for model training"""
        if not self.is_connected:
            logger.error("❌ Not connected to MySQL, cannot retrieve historical data.")
            return pd.DataFrame()
        ...
        try:
            ...
            if records:
                df = pd.DataFrame(records)
                logger.info(f"✅ Retrieved {len(df)} historical records from MySQL")
                return df
            else:
                logger.warning("⚠️ No historical data found in MySQL")
                return pd.DataFrame()

        except Error as e:
            logger.error(f"❌ Error retrieving historical data from MySQL: {e}")
            return pd.DataFrame()

    def get_statistics(self) -> Dict[str, Any]:
        """Get feature store statistics"""
        if not self.is_connected:
            logger.error("❌ Not connected to MySQL, cannot get statistics.")
        ...
        try:
            ...
            logger.error(f"❌ Error getting statistics from MySQL: {e}")
            return {'error': str(e)}

    def clear_data(self) -> bool:
        """Clear all data for Karachi from the feature store"""
        if not self.is_connected:
            logger.error("❌ Not connected to MySQL, cannot clear data.")
            return False

        try:
            cursor = self.connection.cursor()
            cursor.execute(f"DELETE FROM {self.table_name} WHERE city = 'Karachi'")
            logger.info(f"🗑️ Cleared {cursor.rowcount} records for Karachi from MySQL table '{self.table_name}'")
            return True

        except Error as e:
            logger.error(f"❌ Error clearing data from MySQL: {e}")
            return False
...
if __name__ == "__main__":
    ...
    print("🧪 Testing MySQL Feature Store")
    print("=" * 50)

    # Assume a local MySQL instance with database 'aqi_data' and user 'root' with no password
    mysql_config = {
        'host': 'localhost',
        'user': 'root',
        'password': '',
    ...
    store = MySQLFeatureStore(**mysql_config)

    if store.is_connected:
        print("✅ MySQLFeatureStore initialized successfully.")
        ...
        print("\n🎉 MySQL Feature Store test completed!")

    else:
        print("❌ Failed to initialize MySQLFeatureStore.")
